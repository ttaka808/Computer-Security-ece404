...
round_permutation = [15, 6, 19, 20, 28, 11, 27, 16,
...
def decrypt(inputF, keyF, outputF):
    #If I were to implement this it would be made more difficult since there's a need to go through the first three lines in such a way that I'd still be considering the input file as a hexstring
    z = 1+1
    return
...
def main():
    if len(sys.argv) == 4:
        encrypt(inputF=sys.argv[1], keyF=sys.argv[2], outputF=sys.argv[3])
    else:
        print("python3 [script].py [input file].ppm key.txt [output file].txt")

def test():
    print("encrypting image...")
    #encrypt(inputF='image.ppm', keyF='testkey.txt', outputF='outImage.ppm')
    encrypt(inputF='west_2.ppm', keyF='testkey.txt', outputF='fuckingAroundImage.ppm')
